function escapeHtml(value) {
    ...
    (async function() {
      ...
      const COURSE_OVERVIEW = 'Психосоматика здесь рассматривается без мистики и упрощений: как связь напряжения, подавленных эмоций, стресса и телесных симптомов. Курс помогает читать сигналы тела внимательнее, видеть повторяющиеся реакции и находить более зрелые способы регуляции вместо борьбы с собой.';
      ...
      try {
        renderSkeletons(10);
        const lessons_data = await window.API.request('GET', '/content/psihosomatika-lessons');
        lessons = Array.isArray(lessons_data) ? lessons_data : [];
        if (window.CourseAI) await window.CourseAI.init('psihosomatika', lessons);
        ...
        if (lessons[0]) {
          if (window.setupVideoPreview) {
            window.setupVideoPreview(videoContainer, {
              poster: '/assets/webp/psysoma-prew.webp',
              audioPoster: '/assets/webp/psysomatic.webp',
