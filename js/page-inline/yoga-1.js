function escapeHtml(value) {
    ...
    (async function() {
      ...
      const COURSE_OVERVIEW = 'Мини-йога — короткий вход в телесную практику без давления и спортивной гонки. Дыхание, мягкое движение и внимание к ощущениям помогают вернуть контакт с телом, снять лишнее напряжение и собрать спокойную внутреннюю опору на каждый день.';
      ...
      try {
        renderSkeletons(5);
        const lessons_data = await window.API.request('GET', '/content/yoga-lessons');
        lessons = Array.isArray(lessons_data) ? lessons_data : [];
        if (window.CourseAI) await window.CourseAI.init('yoga', lessons);

        courseTitle.textContent = lessons[0]?.title || 'Мини-курс Йога';
        ...
        if (lessons[0]) {
          if (window.setupVideoPreview) {
            ...
            window.setupVideoPreview(videoContainer, {
              poster: '/assets/webp/mini-yoga.webp',
