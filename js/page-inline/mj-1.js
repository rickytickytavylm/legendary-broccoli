function escapeHtml(value) {
    ...
    (async function() {
      ...
      const COURSE_OVERVIEW = 'Курс о мужском и женском как о живой динамике отношений: ожиданиях, близости, притяжении, конфликтах и различиях в восприятии. Материалы помогают увидеть повторяющиеся сценарии и говорить о важном спокойнее, честнее и без игры в угадывание.';
      ...
      try {
        renderSkeletons(9);
        const lessons_data = await window.API.request('GET', '/content/mj-lessons');
        lessons = Array.isArray(lessons_data) ? lessons_data : [];
        if (window.CourseAI) await window.CourseAI.init('mj', lessons);

        courseTitle.textContent = lessons[0] ? lessons[0].title : 'Мужское и Женское';
        ...
        if (lessons[0]) {
          if (window.setupVideoPreview) {
            ...
            window.setupVideoPreview(videoContainer, {
              poster: '/assets/webp/man_woman.webp',
