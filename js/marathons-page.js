...
function getSectionDescription(section) {
  const descriptions = {
    stress: 'Марафон о том, как распознавать стресс, снижать внутреннее напряжение и возвращать себе ощущение управляемости. Подходит как практичный вход в тему саморегуляции и восстановления.',
    ...
    rezakina: 'Практический блок с отдельной авторской логикой и последовательностью уроков. Подходит для самостоятельной работы с конкретной темой через короткий формат.',
    'om-marafon': 'Подборка марафонских материалов "Образ мысли" по стрессу, самооценке и конфликтам. Это быстрый способ вернуться к ключевым темам и выбрать точку входа под текущий запрос.',
  };
...
function getSectionPoster(section) {
  const posters = {
    stress: '/assets/webp/stress-mar.webp',
