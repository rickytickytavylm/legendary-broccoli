(function initTherapyGroupChat() {
  ...
  const chatApi = {
    ...
    getMessages(limit, beforeId, afterId) {
      return window.API.getTherapyGroupMessages(therapyRoute, limit, beforeId, afterId);
    },
    sendMessage(text, opts) {
      return window.API.sendTherapyGroupMessage(therapyRoute, text, opts);
    },
    updateMessage(id, text) {
      return window.API.updateTherapyGroupMessage(therapyRoute, id, text);
    },
    deleteMessage(id) {
      return window.API.deleteTherapyGroupMessage(therapyRoute, id);
    },
    reactMessage(id, emoji) {
      return window.API.reactTherapyGroupMessage(therapyRoute, id, emoji);
    },
    uploadPath() {
      return `${window.API.base}/chat/therapy-groups/${encodeURIComponent(therapyRoute)}/upload`;
    },
    messagesPath() {
      return `/chat/therapy-groups/${encodeURIComponent(therapyRoute)}/messages`;
    },
    socketJoinPayload() {
      return { room: 'therapy', route: therapyRoute, token: window.API.accessToken || '' };
    },
  };
  ...
  list?.addEventListener('click', async (event) => {
    ...
    if (reactionBtn) {
      ...
      if (messageId && emoji) {
        try {
          const data = await chatApi.reactMessage(messageId, emoji);
  ...
  async function loadMessages(silent) {
    try {
      ...
      const data = await chatApi.getMessages(80);
      mergeMessages(data.messages || []);
      setStatus('Терапевтическая группа открыта');
    } catch (err) {
  ...
  async function boot() {
    try {
      ...
      const chatData = await chatApi.getChat();
  ...
  function connectSocket() {
    ...
    state.socket.on('connect', () => {
      state.socket.emit('chat:join', chatApi.socketJoinPayload(), (ack) => {
        if (ack && ack.ok) {
          setStatus('Терапевтическая группа онлайн');
      ...
      if (state.latestId > 0) {
        chatApi.getMessages(100, null, state.latestId)
          .then((res) => {
  ...
  form?.addEventListener('submit', async (event) => {
    ...
    try {
      if (state.editingMessageId) {
        const data = await chatApi.updateMessage(state.editingMessageId, text);
  ...
  const uploadAndSendMedia = async (blob, type, mime) => {
    ...
    try {
      ...
      const response = await fetch(chatApi.uploadPath(), {
        method: 'POST',
        headers: {
      ...
      const sendResponse = await window.API.request('POST', chatApi.messagesPath(), {
        type: msgType,
